...
export async function generateBlog(topic, retries = 3) {
  const prompt = `
    ...
    The response MUST be in strict JSON format with the following structure:
    {
      ...
      "tags": ["tag1", "tag2", "tag3"],
      "image": {
        "prompt": "Highly detailed, cinematic 16:9 illustration for article titled: '[TITLE]'. EXPLICITLY VISUALIZE the core subject from the headline (e.g. if title is about Bitcoin, show a Bitcoin coin). Style: Professional, sharp focus, 8k resolution, cinematic lighting. DO NOT use abstract motherboards.",
        "negative_prompt": "motherboard, circuit board, blurry, low quality, text, watermark, logo, bad anatomy, distorted"
      }
    }
    ...
    - IMAGE PROMPT MUST BE DYNAMIC: Replace [TITLE] with the actual article title and create a UNIQUE visual description for that specific topic.
    - A user MUST be able to tell exactly what the post is about just by looking at the image.
