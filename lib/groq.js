...
export async function generateBlog(topic, retries = 3) {
  const prompt = `
    ...
    The response MUST be in strict JSON format with the following structure:
    {
      ...
      "tags": ["tag1", "tag2", "tag3"],
      "image": {
        "prompt": "Highly detailed, cinematic prompt for a cover image. ALWAYS include style keywords: ultra realistic, cinematic lighting, 4k, high detail",
        "negative_prompt": "blurry, low quality, text, watermark, logo, distorted, bad anatomy"
      }
    }
    ...
    - Image prompt MUST be highly detailed and cinematic, avoid generic terms.
